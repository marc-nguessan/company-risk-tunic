...
- When you use the LLM, use Anthropic tool use for structured output with `tool_choice` forcing the
  tool, `temperature=0`, a pinned configurable model, versioned prompt files, and a
  validate-then-retry-once guardrail with a `max_tokens` cap.
...
- Put the **reviewers' Anthropic API key** in `.env` as `ANTHROPIC_API_KEY` and set
  `ANTHROPIC_MODEL` to whatever model string they specified.
