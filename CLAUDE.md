...
| LLM | **OpenRouter** via the **`openai` Python SDK** (OpenAI-compatible), using **`response_format` json_schema** for structured output |
...
- Feed raw text to the LLM via OpenRouter using **`response_format` with `type: "json_schema"`**.
  The `json_schema.schema` is the JSON schema of a Pydantic model
  `AdverseMediaFinding {is_adverse, category, severity, summary}`, with `strict: true`.
  Generate the schema from the Pydantic model (`model_json_schema()`) so model and schema never drift.
...
The reviewers supply an **OpenRouter** key (prefix `sk-or-v1-`). OpenRouter is an OpenAI-compatible
gateway, so use the `openai` Python SDK pointed at OpenRouter's base URL — NOT the Anthropic SDK.
...
2. **Structured output via `response_format` json_schema**, never prose parsing. Pass
   `response_format={"type": "json_schema", "json_schema": {"name": ..., "strict": True,
   "schema": <PydanticModel>.model_json_schema()}}`. Generate the schema FROM the Pydantic model so
   they can't drift. (Tool/function calling with `tool_choice` forcing a named function is an
   acceptable alternative, but json_schema is simpler here — pick one and be consistent.)
   Confirm the chosen model supports structured outputs on OpenRouter's models page.
3. **Determinism / reproducibility:** `temperature=0`, pinned configurable model via env
   (`LLM_MODEL`, e.g. `anthropic/claude-sonnet-4` or whatever the reviewers specify — OpenRouter
   uses `provider/model` slugs, not bare names), frozen system prompt, deterministically-sorted
   inputs. State in the README that LLMs are not bit-reproducible even at temp 0, so (a) cache by
   input hash, and (b) keep SCORING fully deterministic and LLM-free.
...
5. **Guardrails (satisfies stretch goal):**
   - Validation-and-retry: parse the returned JSON and validate against the Pydantic model; if it
     fails, re-prompt ONCE with the validation error appended, then fall back to a `status="error"`
     SourceResult. (Keep a defensive parser even with json_schema — strip markdown fences, fail
     closed. OpenRouter's optional `response-healing` plugin can also repair imperfect JSON on
     non-streaming calls.)
   - `max_tokens` budget cap on every call.
6. Config comes from env: `OPENROUTER_API_KEY`, `OPENROUTER_BASE_URL`, `LLM_MODEL`. Never hardcode
   the key. OpenRouter also accepts optional `HTTP-Referer` / `X-Title` headers for attribution —
   harmless to include, fine to omit.
...
├── README.md
├── CLAUDE.md
├── .env.example                 # OPENROUTER_API_KEY, OPENROUTER_BASE_URL, LLM_MODEL, CH_API_KEY
...
│   │   ├── config.py
│   │   ├── llm/
│   │   │   ├── client.py        # OpenRouter (openai SDK) wrapper, json_schema, retry, caching
...
3. AdverseMediaSource with json_schema structured output + validation/retry guardrail + prompt versioning.
